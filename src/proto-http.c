        ...
        struct InteractiveData *more)
{
    ...
    for (i=0; i<length; i++) {
        // printf("-- i = %d, state2 = %d, id = %zu, state = %d, log_begin = %d, log_end = %d, px = %c\n", i, state2, id, state, log_begin, log_end, px[i]);
        switch (state) {
            case 0:
            case 1:
            case 2:
            ...
            case FIELD_NAME:
                ...
                if (id == HTTPFIELD_NEWLINE) {
                    state2 = 0;
                    state = FIELD_START;
                } else if (id == SMACK_NOT_FOUND); /* continue here */
            ...
            case FIELD_VALUE:
                ...
                switch (id) {
                    ...
                    case HTTPFIELD_CONTENT_LENGTH:
                        if (isdigit(px[i] & 0xFF)) { ; /*todo: add content length parsing */
                        } else {
                            id = 0;
                        }
                        break;
